...
bool leftPlayerSkill = true;
bool rightPlayerSkill = true;
...
int Game::execute() {
	init("Football game", SDL_WINDOWPOS_CENTERED,
	...
	while (_running) {
		...
		int ticks_per_frame = 1000 / 60; //60FPS
...
bool Game::loadMedia() {
	...
	_assets.addTexture("leftPlatform", "Resources/plank2.bmp");
	...
	_scoreLabel = std::make_unique<Label>("Scoreboard", "Resources/ARLRDBD.TTF", 200, 200, 28, Red);
	_countdownTimer = std::make_unique<Label>("Countdown", "Resources/ARLRDBD.TTF", 200, 200, 28, Red);
	_controlsLabel = std::make_unique<Label>("Controls", "Resources/ARLRDBD.TTF", 200, 200, 14, Red);
	_skillLabel = std::make_unique<Label>("Skill", "Resources/ARLRDBD.TTF", 200, 200, 14, Red);
...
void Game::onEvents(SDL_Event* event) {
	if (event->type == SDL_QUIT) {
		...
		case SDLK_SPACE:
			...
			break;
		case SDLK_j:
			if (leftPlayerSkill == true && _state == GameState::kStart) {
				_ball->reverseOrSpeedBall(0);
				leftPlayerSkill = 0;
			}
			break;
		case SDLK_1:
			if (rightPlayerSkill == true && _state == GameState::kStart) {
				_ball->reverseOrSpeedBall(1);
				rightPlayerSkill = 0;
			}
			break;
		default:
...
// Helper function to assist with the correct bounce physics of the ball when in contact with the (only PLATFORMS)
...
void Game::gameLoop() {
	...
	case GameState::kScoreScreen: {
		int a = 1;
		leftPlayerSkill = rightPlayerSkill = true;
	}break;
	}
}
...
void Game::render() {
	...
	if (_state == GameState::kMainMenu) {
		leftPlayerSkill == true ? _skillLabel->changeText("skill: +1") : _skillLabel->changeText("skill: 0");
		_skillLabel->show(20, 35);

		rightPlayerSkill == true ? _skillLabel->changeText("skill: +1") : _skillLabel->changeText("skill: 0");
		_skillLabel->show(_game_width - 20 - _controlsLabel->getRect().w, 35);


		_controlsLabel->changeText("left-player: W/S");
		_controlsLabel->show(20, 20);

		_controlsLabel->changeText("right-player: UP/DOWN");
		_controlsLabel->show(_game_width - 20 - _controlsLabel->getRect().w, 20);

		_countdownTimer->changeText("SPACE");
	...
	else if (_state == GameState::kStart) {
		leftPlayerSkill == true ? _skillLabel->changeText("skill: +1") : _skillLabel->changeText("skill: 0");
		_skillLabel->show(20, 35);
